...
<div class="row m-2 showkeyfeaturesdiv ">
    <div class="card  col-12">
        <div class="card-header">
            <h4 class="box-title m-b-0">Mini Banner</h4>
        </div>
        <div class="card-body">
                    ...
                    <div class="card">
                        <div class="card-body">
                                ...
                                <div class="form-group col-md-12">
                                  ...
                                  <div class="form-group col-md-12">
                                    <label for="MiniBanner_Image">Choose Image To Dispaly in Banner*</label><span style="color:red;" id="img_error">Image Size Only greater than  or 1170 * 250</span>
                                    <input type="file" class="form-control" id="minibanner" name="minibanner" required><span id="img_error" class="text-danger ms-3"></span>
                                </div>
...
var _URL = window.URL || window.webkitURL;
 var fileInput = document.getElementById('minibanner');
...
img = new Image();
           ...
           img.onload = function() {
            ...
            $.ajax({
                type: "POST",
                url: "<?=base_url()?>admin/Add-MiniBanImage",
                dataType: 'json',
                data: formData,
                contentType: false,
                processData: false,
...
$(document).ready(function() {
    ...
    function load() {
        ...
        $('#Profiletable').DataTable({
            ...
            ajax: {
                method: "GET",
                url: '<?= base_url('admin/getAllminibanner'); ?>',
                dataSrc: 'minibanner',
                // alert(students);
                // alert(dataSrc);
            },
            columns: [{
                ...
                },
                {
                    data: null,
                    render: function(data, type, row) {
                        return '<button class="btn btn-outline-primary btn-sm activebtn" attrid="' + data.id + '"><i class="fa fa-eye"></i></button>\
      ...
      $(document).on('click', '.editbtn', function() {
        ...
        $.ajax({
            method: "POST",
            url: "<?php echo base_url('admin/editminibanner') ?>",
            data: {
                'id': id
            },
            dataType: "JSON",
            success: function(response) {
                ...
                $('.image').html("<img height='100px' src='<?php echo base_url();?>/assets/images/uploads/mini_banner/" + response['image'] + "'>");
    ...
    //   alert('ok');
        ...
        $.ajax({
            url: "<?= base_url() ?>admin/Updateminibanner",
            dataType: 'json',
            method: 'post',
            processData: false,
            contentType: false,
      ...
      $(document).on('click', '.deletebtn', function() {
        ...
        alert(id);
          $.ajax({
            method: "POST",
            url: "<?php echo base_url('admin/miniBannerdelete') ?>",
            data: {
                'id': id,
            },
            success: function(response) {
      ...
      $(document).on('click', '.activebtn', function() {
        ...
        // alert(id);
          $.ajax({
            method: "POST",
            url: "<?php echo base_url('admin/miniBanneractive') ?>",
            data: {
                'id': id,
            },
            success: function(response) {
